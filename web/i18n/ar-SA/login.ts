const translation = {
  pageTitle: 'تسجيل الدخول إلى One Health',
  pageTitleForE: 'مرحبًا، لنبدأ!',
  welcome: '👋 مرحبًا! يرجى تسجيل الدخول للبدء.',
  email: 'عنوان البريد الإلكتروني',
  emailPlaceholder: 'بريدك الإلكتروني',
  password: 'كلمة المرور',
  passwordPlaceholder: 'كلمة المرور الخاصة بك',
  name: 'اسم المستخدم',
  namePlaceholder: 'اسم المستخدم الخاص بك',
  forget: 'نسيت كلمة المرور؟',
  signBtn: 'تسجيل الدخول',
  continueWithCode: 'المتابعة باستخدام الرمز',
  sendVerificationCode: 'إرسال رمز التحقق',
  usePassword: 'استخدام كلمة المرور',
  useVerificationCode: 'استخدام رمز التحقق',
  or: 'أو',
  installBtn: 'إعداد',
  setAdminAccount: 'إعداد حساب مسؤول',
  setAdminAccountDesc: 'أقصى امتيازات لحساب المسؤول، والتي يمكن استخدامها لإنشاء التطبيقات وإدارة مزودي LLM، وما إلى ذلك.',
  createAndSignIn: 'إنشاء وتسجيل الدخول',
  oneMoreStep: 'خطوة واحدة أخرى',
  createSample: 'بناءً على هذه المعلومات، سنقوم بإنشاء تطبيق تجريبي لك',
  invitationCode: 'رمز الدعوة',
  invitationCodePlaceholder: 'رمز الدعوة الخاص بك',
  interfaceLanguage: 'لغة الواجهة',
  timezone: 'المنطقة الزمنية',
  go: 'انتقل إلى One Health',
  sendUsMail: 'أرسل لنا مقدمتك عبر البريد الإلكتروني، وسنتعامل مع طلب الدعوة.',
  acceptPP: 'لقد قرأت وقبلت سياسة الخصوصية',
  reset: 'يرجى تشغيل الأمر التالي لإعادة تعيين كلمة المرور الخاصة بك',
  withGitHub: 'المتابعة باستخدام GitHub',
  withGoogle: 'المتابعة باستخدام Google',
  withSSO: 'المتابعة باستخدام SSO',
  rightTitle: 'أطلق العنان للإمكانات الكاملة لـ LLM',
  rightDesc: 'قم ببناء تطبيقات الذكاء الاصطناعي الجذابة بصريًا والقابلة للتشغيل والتحسين بسهولة.',
  tos: 'شروط الخدمة',
  pp: 'سياسة الخصوصية',
  tosDesc: 'بالتسجيل، أنت توافق على',
  goToInit: 'إذا لم تقم بتهيئة الحساب، يرجى الانتقال إلى صفحة التهيئة',
  dontHave: 'ليس لديك حساب؟',
  invalidInvitationCode: 'رمز دعوة غير صالح',
  accountAlreadyInited: 'تم تهيئة الحساب بالفعل',
  forgotPassword: 'نسيت كلمة المرور؟',
  resetLinkSent: 'تم إرسال رابط إعادة التعيين',
  sendResetLink: 'إرسال رابط إعادة التعيين',
  backToSignIn: 'العودة لتسجيل الدخول',
  forgotPasswordDesc: 'يرجى إدخال عنوان بريدك الإلكتروني لإعادة تعيين كلمة المرور. سنرسل لك بريدًا إلكترونيًا يحتوي على تعليمات حول كيفية إعادة تعيين كلمة المرور.',
  checkEmailForResetLink: 'يرجى التحقق من بريدك الإلكتروني للحصول على رابط لإعادة تعيين كلمة المرور. إذا لم يظهر في غضون بضع دقائق، فتأكد من التحقق من مجلد الرسائل غير المرغوب فيها.',
  passwordChanged: 'سجل الدخول الآن',
  changePassword: 'تعيين كلمة مرور',
  changePasswordTip: 'يرجى إدخال كلمة مرور جديدة لحسابك',
  changePasswordBtn: 'تعيين كلمة مرور',
  invalidToken: 'رمز غير صالح أو منتهي الصلاحية',
  confirmPassword: 'تأكيد كلمة المرور',
  confirmPasswordPlaceholder: 'تأكيد كلمة المرور الجديدة',
  passwordChangedTip: 'تم تغيير كلمة المرور الخاصة بك بنجاح',
  error: {
    emailEmpty: 'عنوان البريد الإلكتروني مطلوب',
    emailInValid: 'يرجى إدخال عنوان بريد إلكتروني صالح',
    nameEmpty: 'الاسم مطلوب',
    passwordEmpty: 'كلمة المرور مطلوبة',
    passwordLengthInValid: 'يجب أن تتكون كلمة المرور من 8 أحرف على الأقل',
    passwordInvalid: 'يجب أن تحتوي كلمة المرور على أحرف وأرقام، ويجب أن يكون الطول أكبر من 8',
    registrationNotAllowed: 'الحساب غير موجود. يرجى الاتصال بمسؤول النظام للتسجيل.',
    invalidEmailOrPassword: 'البريد الإلكتروني أو كلمة المرور غير صالحة.',
  },
  license: {
    tip: 'قبل بدء إصدار مجتمع One Health، اقرأ GitHub',
    link: 'ترخيص مفتوح المصدر',
  },
  join: 'انضم ',
  joinTipStart: 'أدعوك للانضمام إلى فريق ',
  joinTipEnd: ' على One Health',
  invalid: 'انتهت صلاحية الرابط',
  explore: 'استكشف One Health',
  activatedTipStart: 'لقد انضممت إلى فريق',
  activatedTipEnd: '',
  activated: 'سجل الدخول الآن',
  adminInitPassword: 'كلمة مرور تهيئة المسؤول',
  validate: 'تحقق',
  checkCode: {
    checkYourEmail: 'تحقق من بريدك الإلكتروني',
    tipsPrefix: 'نرسل رمز تحقق إلى ',
    validTime: 'تذكر أن الرمز صالح لمدة 5 دقائق',
    verificationCode: 'رمز التحقق',
    verificationCodePlaceholder: 'أدخل الرمز المكون من 6 أرقام',
    verify: 'تحقق',
    didNotReceiveCode: 'لم تتلق الرمز؟ ',
    resend: 'إعادة الإرسال',
    useAnotherMethod: 'استخدام طريقة أخرى',
    emptyCode: 'الرمز مطلوب',
    invalidCode: 'رمز غير صالح',
  },
  resetPassword: 'إعادة تعيين كلمة المرور',
  resetPasswordDesc: 'اكتب البريد الإلكتروني الذي استخدمته للتسجيل في One Health وسنرسل لك بريدًا إلكترونيًا لإعادة تعيين كلمة المرور.',
  backToLogin: 'العودة لتسجيل الدخول',
  setYourAccount: 'إعداد حسابك',
  enterYourName: 'يرجى إدخال اسم المستخدم الخاص بك',
  back: 'عودة',
  noLoginMethod: 'طريقة المصادقة غير مكونة',
  noLoginMethodTip: 'يرجى الاتصال بمسؤول النظام لإضافة طريقة مصادقة.',
  licenseExpired: 'انتهت صلاحية الترخيص',
  licenseExpiredTip: 'انتهت صلاحية ترخيص One Health Enterprise لمساحة العمل الخاصة بك. يرجى الاتصال بالمسؤول لمتابعة استخدام One Health.',
  licenseLost: 'فقدان الترخيص',
  licenseLostTip: 'فشل الاتصال بخادم ترخيص One Health. يرجى الاتصال بالمسؤول لمتابعة استخدام One Health.',
  licenseInactive: 'الترخيص غير نشط',
  licenseInactiveTip: 'ترخيص One Health Enterprise لمساحة العمل الخاصة بك غير نشط. يرجى الاتصال بالمسؤول لمتابعة استخدام One Health.',
  webapp: {
    login: 'تسجيل الدخول',
    noLoginMethod: 'طريقة المصادقة غير مكونة لتطبيق الويب',
    noLoginMethodTip: 'يرجى الاتصال بمسؤول النظام لإضافة طريقة مصادقة.',
    disabled: 'مصادقة تطبيق الويب معطلة. يرجى الاتصال بمسؤول النظام لتمكينها. يمكنك محاولة استخدام التطبيق مباشرة.',
  },
  signup: {
    noAccount: 'ليس لديك حساب؟ ',
    signUp: 'تسجيل',
    createAccount: 'أنشئ حسابك',
    welcome: '👋 مرحبًا! يرجى ملء التفاصيل للبدء.',
    verifyMail: 'المتابعة باستخدام رمز التحقق',
    haveAccount: 'لديك حساب بالفعل؟ ',
    signIn: 'تسجيل الدخول',
  },
}

export default translation
