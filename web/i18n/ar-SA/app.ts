const translation = {
  createApp: 'إنشاء تطبيق',
  types: {
    all: 'الكل',
    chatbot: 'روبوت الدردشة',
    agent: 'وكيل',
    workflow: 'سير العمل',
    completion: 'إكمال',
    advanced: 'تدفق الدردشة',
    basic: 'أساسي',
  },
  duplicate: 'تكرار',
  mermaid: {
    handDrawn: 'رسم يدوي',
    classic: 'كلاسيكي',
  },
  duplicateTitle: 'تكرار التطبيق',
  export: 'تصدير DSL',
  exportFailed: 'فشل تصدير DSL.',
  importDSL: 'استيراد ملف DSL',
  createFromConfigFile: 'إنشاء من ملف DSL',
  importFromDSL: 'استيراد من DSL',
  importFromDSLFile: 'من ملف DSL',
  importFromDSLUrl: 'من عنوان URL',
  importFromDSLUrlPlaceholder: 'الصق رابط DSL هنا',
  dslUploader: {
    button: 'اسحب وأفلت الملف، أو',
    browse: 'تصفح',
  },
  deleteAppConfirmTitle: 'حذف هذا التطبيق؟',
  deleteAppConfirmContent:
    'حذف التطبيق لا رجعة فيه. لن يتمكن المستخدمون من الوصول إلى تطبيقك، وسيتم حذف جميع تكوينات المطالبات والسجلات نهائيًا.',
  appDeleted: 'تم حذف التطبيق',
  appDeleteFailed: 'فشل حذف التطبيق',
  join: 'One Health AI',
  communityIntro:
    'يوفر كل ما تحتاجه — سير عمل الوكلاء، وخطوط أنابيب RAG، والتكاملات، وقابلية المراقبة — كل ذلك في مكان واحد، مما يضع قوة الذكاء الاصطناعي بين يديك.',
  roadmap: 'شاهد خارطة الطريق لدينا',
  newApp: {
    learnMore: 'اعرف المزيد',
    startFromBlank: 'إنشاء من الصفر',
    startFromTemplate: 'إنشاء من قالب',
    foundResult: '{{count}} نتيجة',
    foundResults: '{{count}} نتائج',
    noAppsFound: 'لم يتم العثور على تطبيقات',
    noTemplateFound: 'لم يتم العثور على قوالب',
    noTemplateFoundTip: 'حاول البحث باستخدام كلمات رئيسية مختلفة.',
    chatbotShortDescription: 'روبوت دردشة قائم على LLM مع إعداد بسيط',
    chatbotUserDescription: 'قم ببناء روبوت دردشة قائم على LLM بسرعة مع تكوين بسيط. يمكنك التبديل إلى Chatflow لاحقًا.',
    completionShortDescription: 'مساعد AI لمهام توليد النص',
    completionUserDescription: 'قم ببناء مساعد AI لمهام توليد النص بسرعة مع تكوين بسيط.',
    agentShortDescription: 'وكيل ذكي مع التفكير واستخدام الأدوات المستقلة',
    agentUserDescription: 'وكيل ذكي قادر على التفكير التكراري واستخدام الأدوات المستقلة لتحقيق أهداف المهمة.',
    workflowShortDescription: 'تدفق وكيل للأتمتة الذكية',
    workflowUserDescription: 'قم ببناء سير عمل AI مستقل بصريًا مع بساطة السحب والإفلات.',
    workflowWarning: 'حاليًا في مرحلة بيتا',
    advancedShortDescription: 'سير عمل محسن للدردشات متعددة الأدوار',
    advancedUserDescription: 'سير عمل مع ميزات ذاكرة إضافية وواجهة روبوت دردشة.',
    chooseAppType: 'اختر نوع التطبيق',
    forBeginners: 'أنواع تطبيقات أكثر أساسية',
    forAdvanced: 'للمستخدمين المتقدمين',
    noIdeaTip: 'ليس لديك أفكار؟ تحقق من قوالبنا',
    captionName: 'اسم التطبيق والأيقونة',
    appNamePlaceholder: 'امنح تطبيقك اسمًا',
    captionDescription: 'الوصف',
    optional: 'اختياري',
    appDescriptionPlaceholder: 'أدخل وصف التطبيق',
    useTemplate: 'استخدم هذا القالب',
    previewDemo: 'معاينة العرض التوضيحي',
    chatApp: 'مساعد',
    chatAppIntro:
      'أريد بناء تطبيق قائم على الدردشة. يستخدم هذا التطبيق تنسيق سؤال وجواب، مما يسمح بجولات متعددة من المحادثة المستمرة.',
    agentAssistant: 'مساعد وكيل جديد',
    completeApp: 'مولد النص',
    completeAppIntro:
      'أريد إنشاء تطبيق يولد نصًا عالي الجودة بناءً على المطالبات، مثل إنشاء المقالات والملخصات والترجمات والمزيد.',
    showTemplates: 'أريد الاختيار من قالب',
    hideTemplates: 'العودة إلى اختيار الوضع',
    Create: 'إنشاء',
    Cancel: 'إلغاء',
    Confirm: 'تأكيد',
    import: 'استيراد',
    nameNotEmpty: 'لا يمكن أن يكون الاسم فارغًا',
    appTemplateNotSelected: 'يرجى تحديد قالب',
    appTypeRequired: 'يرجى تحديد نوع التطبيق',
    appCreated: 'تم إنشاء التطبيق',
    caution: 'تحذير',
    appCreateDSLWarning: 'تحذير: قد يؤثر اختلاف إصدار DSL على ميزات معينة',
    appCreateDSLErrorTitle: 'عدم توافق الإصدار',
    appCreateDSLErrorPart1: 'تم اكتشاف اختلاف كبير في إصدارات DSL. قد يؤدي فرض الاستيراد إلى تعطل التطبيق.',
    appCreateDSLErrorPart2: 'هل تريد المتابعة؟',
    appCreateDSLErrorPart3: 'إصدار DSL للتطبيق الحالي: ',
    appCreateDSLErrorPart4: 'إصدار DSL المدعوم من النظام: ',
    appCreateFailed: 'فشل إنشاء التطبيق',
    dropDSLToCreateApp: 'أفلت ملف DSL هنا لإنشاء التطبيق',
  },
  newAppFromTemplate: {
    byCategories: 'حسب الفئات',
    searchAllTemplate: 'بحث في جميع القوالب...',
    sidebar: {
      Recommended: 'موصى به',
      Agent: 'وكيل',
      Assistant: 'مساعد',
      HR: 'الموارد البشرية',
      Workflow: 'سير العمل',
      Writing: 'الكتابة',
      Programming: 'البرمجة',
    },
  },
  editApp: 'تعديل المعلومات',
  editAppTitle: 'تعديل معلومات التطبيق',
  editDone: 'تم تحديث معلومات التطبيق',
  editFailed: 'فشل تحديث معلومات التطبيق',
  iconPicker: {
    ok: 'موافق',
    cancel: 'إلغاء',
    emoji: 'رموز تعبيرية',
    image: 'صورة',
  },
  answerIcon: {
    title: 'استخدم أيقونة تطبيق الويب لاستبدال 🤖',
    description: 'ما إذا كان سيتم استخدام أيقونة تطبيق الويب لاستبدال 🤖 في التطبيق المشترك',
    descriptionInExplore: 'ما إذا كان سيتم استخدام أيقونة تطبيق الويب لاستبدال 🤖 في الاستكشاف',
  },
  switch: 'التبديل إلى تنسيق سير العمل',
  switchTipStart: 'سيتم إنشاء نسخة تطبيق جديدة لك، وستنتقل النسخة الجديدة إلى تنسيق سير العمل. النسخة الجديدة ستكون ',
  switchTip: 'غير مسموح',
  switchTipEnd: ' بالتبديل مرة أخرى إلى التنسيق الأساسي.',
  switchLabel: 'نسخة التطبيق التي سيتم إنشاؤها',
  removeOriginal: 'حذف التطبيق الأصلي',
  switchStart: 'بدء التبديل',
  openInExplore: 'فتح في الاستكشاف',
  typeSelector: {
    all: 'جميع الأنواع ',
    chatbot: 'روبوت الدردشة',
    agent: 'وكيل',
    workflow: 'سير العمل',
    completion: 'إكمال',
    advanced: 'تدفق الدردشة',
  },
  tracing: {
    title: 'تتبع أداء التطبيق',
    description: 'تكوين مزود LLMOps تابع لجهة خارجية وتتبع أداء التطبيق.',
    config: 'تكوين',
    view: 'عرض',
    collapse: 'طي',
    expand: 'توسيع',
    tracing: 'التتبع',
    disabled: 'معطل',
    disabledTip: 'يرجى تكوين المزود أولاً',
    enabled: 'في الخدمة',
    tracingDescription: 'التقاط السياق الكامل لتنفيذ التطبيق، بما في ذلك استدعاءات LLM، والسياق، والمطالبات، وطلبات HTTP، والمزيد، إلى منصة تتبع تابعة لجهة خارجية.',
    configProviderTitle: {
      configured: 'تم التكوين',
      notConfigured: 'تكوين المزود لتمكين التتبع',
      moreProvider: 'المزيد من المزودين',
    },
    arize: {
      title: 'Arize',
      description: 'مراقبة LLM على مستوى المؤسسة، والتقييم عبر الإنترنت وغير متصل، والمراقبة، والتجريب - مدعوم من OpenTelemetry. مصمم خصيصًا لتطبيقات LLM والتطبيقات التي تعتمد على الوكيل.',
    },
    phoenix: {
      title: 'Phoenix',
      description: 'منصة مراقبة وتقييم وهندسة المطالبات والتجريب مفتوحة المصدر وقائمة على OpenTelemetry لسير عمل LLM والوكلاء.',
    },
    langsmith: {
      title: 'LangSmith',
      description: 'منصة مطور شاملة لكل خطوة من دورة حياة التطبيق المدعوم بـ LLM.',
    },
    langfuse: {
      title: 'Langfuse',
      description: 'مراقبة LLM مفتوحة المصدر، والتقييم، وإدارة المطالبات والمقاييس لتصحيح وتحسين تطبيق LLM الخاص بك.',
    },
    opik: {
      title: 'Opik',
      description: 'Opik هي منصة مفتوحة المصدر لتقييم واختبار ومراقبة تطبيقات LLM.',
    },
    weave: {
      title: 'Weave',
      description: 'Weave هي منصة مفتوحة المصدر لتقييم واختبار ومراقبة تطبيقات LLM.',
    },
    aliyun: {
      title: 'Cloud Monitor',
      description: 'منصة المراقبة المدارة بالكامل والخالية من الصيانة المقدمة من Alibaba Cloud، تتيح المراقبة والتتبع والتقييم الجاهز لتطبيقات One Health.',
    },
    mlflow: {
      title: 'MLflow',
      description: 'MLflow هي منصة مفتوحة المصدر لإدارة التجارب والتقييم ومراقبة تطبيقات LLM.',
    },
    databricks: {
      title: 'Databricks',
      description: 'تقدم Databricks تدفق MLflow مدار بالكامل مع حوكمة وأمان قويين لتخزين بيانات التتبع.',
    },
    tencent: {
      title: 'Tencent APM',
      description: 'يوفر Tencent Application Performance Monitoring تتبعًا شاملاً وتحليلاً متعدد الأبعاد لتطبيقات LLM.',
    },
    inUse: 'قيد الاستخدام',
    configProvider: {
      title: 'تكوين ',
      placeholder: 'أدخل {{key}} الخاص بك',
      project: 'المشروع',
      trackingUri: 'URI التتبع',
      experimentId: 'معرف التجربة',
      username: 'اسم المستخدم',
      password: 'كلمة المرور',
      publicKey: 'المفتاح العام',
      secretKey: 'المفتاح السري',
      viewDocsLink: 'عرض وثائق {{key}}',
      removeConfirmTitle: 'إزالة تكوين {{key}}؟',
      removeConfirmContent: 'التكوين الحالي قيد الاستخدام، ستؤدي إزالته إلى إيقاف تشغيل ميزة التتبع.',
      clientId: 'معرف عميل OAuth',
      clientSecret: 'سر عميل OAuth',
      personalAccessToken: 'رمز الوصول الشخصي (قديم)',
      databricksHost: 'عنوان URL لمساحة عمل Databricks',
    },
  },
  appSelector: {
    label: 'التطبيق',
    placeholder: 'حدد تطبيقًا...',
    params: 'معلمات التطبيق',
    noParams: 'لا توجد معلمات مطلوبة',
  },
  showMyCreatedAppsOnly: 'تم إنشاؤه بواسطتي',
  structOutput: {
    moreFillTip: 'عرض 10 مستويات كحد أقصى من التداخل',
    required: 'مطلوب',
    LLMResponse: 'استجابة LLM',
    configure: 'تكوين',
    notConfiguredTip: 'لم يتم تكوين الإخراج الهيكلي بعد',
    structured: 'هيكلي',
    structuredTip: 'المخرجات الهيكلية هي ميزة تضمن أن النموذج سيولد دائمًا استجابات تلتزم بمخطط JSON المقدم منك',
    modelNotSupported: 'النموذج غير مدعوم',
    modelNotSupportedTip: 'النموذج الحالي لا يدعم هذه الميزة ويتم تخفيضه تلقائيًا لحقن المطالبة.',
  },
  accessControl: 'التحكم في الوصول إلى تطبيق الويب',
  accessItemsDescription: {
    anyone: 'يمكن لأي شخص الوصول إلى تطبيق الويب (لا يلزم تسجيل الدخول)',
    specific: 'يمكن لأعضاء محددين داخل المنصة الوصول إلى تطبيق الويب',
    organization: 'يمكن لجميع الأعضاء داخل المنصة الوصول إلى تطبيق الويب',
    external: 'يمكن للمستخدمين الخارجيين المصادق عليهم فقط الوصول إلى تطبيق الويب',
  },
  accessControlDialog: {
    title: 'التحكم في الوصول إلى تطبيق الويب',
    description: 'تعيين أذونات الوصول إلى تطبيق الويب',
    accessLabel: 'من لديه حق الوصول',
    accessItems: {
      anyone: 'أي شخص لديه الرابط',
      specific: 'أعضاء محددون داخل المنصة',
      organization: 'جميع الأعضاء داخل المنصة',
      external: 'المستخدمون الخارجيون المصادق عليهم',
    },
    groups_one: '{{count}} مجموعة',
    groups_other: '{{count}} مجموعات',
    members_one: '{{count}} عضو',
    members_other: '{{count}} أعضاء',
    noGroupsOrMembers: 'لم يتم تحديد مجموعات أو أعضاء',
    webAppSSONotEnabledTip: 'يرجى الاتصال بمسؤول مؤسستك لتكوين المصادقة الخارجية لتطبيق الويب.',
    operateGroupAndMember: {
      searchPlaceholder: 'البحث عن المجموعات والأعضاء',
      allMembers: 'جميع الأعضاء',
      expand: 'توسيع',
      noResult: 'لا توجد نتائج',
    },
    updateSuccess: 'تم التحديث بنجاح',
  },
  publishApp: {
    title: 'من يمكنه الوصول إلى تطبيق الويب',
    notSet: 'لم يتم التعيين',
    notSetDesc: 'حاليًا لا يمكن لأحد الوصول إلى تطبيق الويب. يرجى تعيين الأذونات.',
  },
  noAccessPermission: 'لا يوجد إذن للوصول إلى تطبيق الويب',
  noUserInputNode: 'عقدة إدخال المستخدم مفقودة',
  notPublishedYet: 'التطبيق لم ينشر بعد',
  maxActiveRequests: 'الحد الأقصى للطلبات المتزامنة',
  maxActiveRequestsPlaceholder: 'أدخل 0 لغير محدود',
  maxActiveRequestsTip: 'الحد الأقصى لعدد الطلبات النشطة المتزامنة لكل تطبيق (0 لغير محدود)',
  gotoAnything: {
    searchPlaceholder: 'ابحث أو اكتب @ أو / للأوامر...',
    searchTitle: 'ابحث عن أي شيء',
    searching: 'جاري البحث...',
    noResults: 'لم يتم العثور على نتائج',
    searchFailed: 'فشل البحث',
    searchTemporarilyUnavailable: 'البحث غير متاح مؤقتًا',
    servicesUnavailableMessage: 'قد تواجه بعض خدمات البحث مشكلات. حاول مرة أخرى بعد قليل.',
    someServicesUnavailable: 'بعض خدمات البحث غير متاحة',
    resultCount: '{{count}} نتيجة',
    resultCount_other: '{{count}} نتائج',
    inScope: 'في {{scope}}s',
    clearToSearchAll: 'امسح @ للبحث في الكل',
    useAtForSpecific: 'استخدم @ لأنواع محددة',
    selectToNavigate: 'حدد للتنقل',
    startTyping: 'ابدأ الكتابة للبحث',
    tips: 'اضغط ↑↓ للتنقل',
    pressEscToClose: 'اضغط ESC للإغلاق',
    selectSearchType: 'اختر ما تريد البحث عنه',
    searchHint: 'ابدأ الكتابة للبحث عن كل شيء على الفور',
    commandHint: 'اكتب @ للتصفح حسب الفئة',
    slashHint: 'اكتب / لرؤية جميع الأوامر المتاحة',
    actions: {
      searchApplications: 'البحث في التطبيقات',
      searchApplicationsDesc: 'ابحث وانتقل إلى تطبيقاتك',
      searchPlugins: 'البحث في الإضافات',
      searchPluginsDesc: 'ابحث وانتقل إلى إضافاتك',
      searchKnowledgeBases: 'البحث في قواعد المعرفة',
      searchKnowledgeBasesDesc: 'ابحث وانتقل إلى قواعد المعرفة الخاصة بك',
      searchWorkflowNodes: 'البحث في عقد سير العمل',
      searchWorkflowNodesDesc: 'ابحث وانتقل إلى العقد في سير العمل الحالي بالاسم أو النوع',
      searchWorkflowNodesHelp: 'تعمل هذه الميزة فقط عند عرض سير العمل. انتقل إلى سير العمل أولاً.',
      runTitle: 'الأوامر',
      runDesc: 'تشغيل أوامر سريعة (السمة، اللغة، ...)',
      themeCategoryTitle: 'السمة',
      themeCategoryDesc: 'تبديل سمة التطبيق',
      themeSystem: 'سمة النظام',
      themeSystemDesc: 'اتبع مظهر نظام التشغيل الخاص بك',
      themeLight: 'السمة الفاتحة',
      themeLightDesc: 'استخدم المظهر الفاتح',
      themeDark: 'السمة الداكنة',
      themeDarkDesc: 'استخدم المظهر الداكن',
      languageCategoryTitle: 'اللغة',
      languageCategoryDesc: 'تبديل لغة الواجهة',
      languageChangeDesc: 'تغيير لغة واجهة المستخدم',
      slashDesc: 'تنفيذ الأوامر (اكتب / لرؤية جميع الأوامر المتاحة)',
      accountDesc: 'الانتقال إلى صفحة الحساب',
      communityDesc: 'فتح مجتمع Discord',
      docDesc: 'فتح وثائق المساعدة',
      feedbackDesc: 'فتح مناقشات ملاحظات المجتمع',
      zenTitle: 'وضع Zen',
      zenDesc: 'تبديل وضع تركيز اللوحة',
    },
    emptyState: {
      noAppsFound: 'لم يتم العثور على تطبيقات',
      noPluginsFound: 'لم يتم العثور على إضافات',
      noKnowledgeBasesFound: 'لم يتم العثور على قواعد معرفة',
      noWorkflowNodesFound: 'لم يتم العثور على عقد سير العمل',
      tryDifferentTerm: 'حاول استخدام مصطلح بحث مختلف',
      trySpecificSearch: 'حاول {{shortcuts}} لعمليات بحث محددة',
    },
    groups: {
      apps: 'التطبيقات',
      plugins: 'الإضافات',
      knowledgeBases: 'قواعد المعرفة',
      workflowNodes: 'عقد سير العمل',
      commands: 'الأوامر',
    },
    noMatchingCommands: 'لم يتم العثور على أوامر مطابقة',
    tryDifferentSearch: 'حاول استخدام مصطلح بحث مختلف',
  },
}

export default translation
